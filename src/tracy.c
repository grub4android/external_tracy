...
int tracy_debug_current(struct tracy_child *child) {
    ...
    printf("DEBUG: 0: %ld 1: %ld 2: %ld 3: %ld 4: %ld 5: %ld"
            ...
            get_reg(&a, 0, abi), get_reg(&a, 1, abi), get_reg(&a, 2, abi),
            get_reg(&a, 3, abi), get_reg(&a, 4, abi), get_reg(&a, 5, abi),
            (unsigned long)a.TRACY_SYSCALL_REGISTER, (unsigned long)a.TRACY_RETURN_CODE,
            (unsigned long)a.TRACY_IP_REG, (unsigned long)a.TRACY_STACK_POINTER
            );
    ...
    printf("DEBUG: 0: %lx 1: %lx 2: %lx 3: %lx 4: %lx 5: %lx"
            ...
            get_reg(&a, 0, abi), get_reg(&a, 1, abi), get_reg(&a, 2, abi),
            get_reg(&a, 3, abi), get_reg(&a, 4, abi), get_reg(&a, 5, abi),
            (long)a.TRACY_SYSCALL_REGISTER, (long)a.TRACY_RETURN_CODE,
            (long)a.TRACY_IP_REG, (long)a.TRACY_STACK_POINTER
            );
#else
    printf("DEBUG: 0: %ld 1: %ld 2: %ld 3: %ld 4: %ld 5: %ld"
            " s: %Lu, R: %Lu, PC: %Lu SP: %Lu\n",
